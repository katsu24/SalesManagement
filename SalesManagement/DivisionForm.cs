...
using System.Data.SqlClient;            /* データプロバイダー名前空間インポート */
...
namespace SalesManagement
{
    ...
    {   //メッセージ表示用クラスのインスタンス化
        ...
        private void buttonImport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (str == "1")
                {
                    var context = new SalesContext();
                    //M_Divisionテーブルを再作成
                    context.Database.ExecuteSqlCommand("TRUNCATE TABLE M_Division; ");
                    context.SaveChanges();
                }
                //インポートするCSVファイルの指定
                string csvpth = System.Environment.CurrentDirectory + "\\Division.csv";
                //データテーブルの設定
        ...
        {
            //部署IDの有無をチェック
            if(textBoxDivisionID.Text=="" || textBoxDivisionID.Text == null)
            {
                fncButtonEnable(1);
            }
            else
        ...
        private bool fncAddInputCheck()
        {
            DialogResult result;
            //部署名の入力データ有無チェック
            if ((textBoxDivisionName.Text).Trim() == "" || textBoxDivisionName.Text == null)
            {
                //入力無しエラーメッセージの表示
                result = msg.MsgDsp("M10009");
                textBoxDivisionName.Focus();
                return false;
            }
            //
            //部署名の文字数チェック
            if (Encoding.GetEncoding("Shift_JIS").GetByteCount(textBoxDivisionName.Text) > 50)
            {
                //文字数エラーメッセージの表示
                result = msg.MsgDsp("M10010");
                textBoxDivisionName.Focus();
                return false;
            }
